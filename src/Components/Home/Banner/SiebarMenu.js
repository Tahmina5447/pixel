...
const SiebarMenu = ({ catagories }) => {
  ...
  console.log("----------hello", catagories?.data?.result)
  return (
    <div className="">
        ...
        <>
          {
            isLoading ? <CustomCategorySkeleton /> : <>{catagories?.data?.result?.slice(0, 8).map((category) => {
              const isParentCategoryActive =
                activeParentCategory === category?.parent_category;
              const hasChildCategories = category?.first_child_Category?.length > 0;
              return (
                <li
                  key={category._id}
                  id="parent-category"
                  className="shadow-md avenir2"
                  onMouseEnter={() =>
                  ...
                  {hasChildCategories && (
                    <ul
                      ...
                      className={`${isParentCategoryActive ? "visible" : "hidden"
                        } absolute mt-2 min-h-[250px] shadow-md py-2`}
                    >
                      {category.first_child_Category?.map((child, index) => (
                        <li key={index}>
                          <Link
                            ...
                            onMouseEnter={() =>
                              handleChildCategoryMouseEnter(category.firstChildTitle)
                            }
